...
void imu_setup()
{
  ...
  if (id != 0x6C) {
    while (1) {
      mcu_log_error("IMU ID mismatch: expected 0x6C, got {}\n", id);
      delay(100);
    }
  }
...
//	attachInterrupt(digitalPinToInterrupt(IMU_INT1), imu_fifo_interrupt,
//RISING);
#endif
        ...
        for (int t = 0; t < cal_time_ms;) {
          ...
          if (imu_get_sample(&sample) == 0) {
            // TODO: should be a.z*cos(alpha), this assumes board is perfectly
            // horizontal
...
int imu_get_sample(FIFO_Sample * sample)
      {
        ...
        for (int i = 0; i < n_samples; i++) {
          ...
          case LSM6DSO32_XL_NC_TAG:
            ...
            xx_ov |= __builtin_add_overflow(avg_sample.accelerometer[0],
                                            tmp_sample.accelerometer[0],
                                            &avg_sample.accelerometer[0]);
            xy_ov |= __builtin_add_overflow(avg_sample.accelerometer[1],
                                            tmp_sample.accelerometer[1],
                                            &avg_sample.accelerometer[1]);
            xz_ov |= __builtin_add_overflow(avg_sample.accelerometer[2],
                                            tmp_sample.accelerometer[2],
                                            &avg_sample.accelerometer[2]);
            x_count++;
            break;
          case LSM6DSO32_GYRO_NC_TAG:
            IMU.Get_FIFO_G_Axes(tmp_sample.gyroscope);
            gx_ov |= __builtin_add_overflow(avg_sample.gyroscope[0],
                                            tmp_sample.gyroscope[0],
                                            &avg_sample.gyroscope[0]);
            gy_ov |= __builtin_add_overflow(avg_sample.gyroscope[1],
                                            tmp_sample.gyroscope[1],
                                            &avg_sample.gyroscope[1]);
            gz_ov |= __builtin_add_overflow(avg_sample.gyroscope[2],
                                            tmp_sample.gyroscope[2],
                                            &avg_sample.gyroscope[2]);
            g_count++;
            break;
