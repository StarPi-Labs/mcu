    ...
    while (true) {
        {
            ...
            // 2. Abbiamo un messaggio. Aspettiamo fino a 100ms per raggruppare altri messaggi,
            // a meno che il buffer non si stia riempiendo (es. raggiunge l'80% della capacità)
            using namespace std::chrono_literals;
            
            g_logBuffer.cvBufferEmpty.wait_for(lock, 100ms, [&]() {
                constexpr std::size_t CAPACITY_THRESHOLD = (MCU_LOG_BUFFER_SIZE * 8) / 10;
            ...
            for (std::size_t i = 0; i < g_logHandlers.size(); ++i) {
                ...
                bool result = xTaskCreate([](void* param) -> void {
                    HandlerParam* hp = reinterpret_cast<HandlerParam*>(param);
                    
                    g_logHandlers[hp->index](std::string_view(g_logBuffer.data.data(), g_logBuffer.length));
                    
                    xTaskNotifyGive(hp->loggerTask);
                    vTaskDelete(nullptr);
                }, "LogHandler", configMINIMAL_STACK_SIZE, reinterpret_cast<void*>(&params[i]), tskIDLE_PRIORITY + 1, &handlerTasks[i]) == pdTRUE;
