/*
 * TODO:
 * 1. Make get_next_packet() independent of the packet type so that both the GS
 * and FC can use the same mechanism
 ...
 *
 * FUTURE IMPROVEMENTS:
 * 1. The protocol can be simplified by employing slotted CSMA, since there are
 * only two peers (the GS and FC) congestion should be manageable
...
// RX commands will be pushed to this queue and the handler task will be
// signaled
...
const struct BandRequirements EU_868_BANDS[] = {
    [BAND_K] = {.freq_start_mhz = 863.0,
                .ch_bw_khz = 500,
                .num_channels = 4,
                .max_power_mw = 25,
                .max_duty = 0.1,
                .polite_access = true},
    [BAND_L] = {.freq_start_mhz = 865.0,
                .ch_bw_khz = 500,
                .num_channels = 6,
                .max_power_mw = 25,
                .max_duty = 1,
                .polite_access = true},
    [BAND_M] = {.freq_start_mhz = 868.0,
                .ch_bw_khz = 500,
                .num_channels = 1,
                .max_power_mw = 25,
                .max_duty = 1,
                .polite_access = true},
    [BAND_N] = {.freq_start_mhz = 868.7,
                .ch_bw_khz = 500,
                .num_channels = 1,
                .max_power_mw = 25,
                .max_duty = 0.1,
                .polite_access = true},
    [BAND_O] = {.freq_start_mhz = 869.4,
                .ch_bw_khz = 250,
                .num_channels = 1,
                .max_power_mw = 500,
                .max_duty = 10,
...
// Refer to image in page 10, chapter 4.2 of the manual:
// https://www.cdebyte.com/products/E22-900M33S/4#Downloads Useful to extract
// data: https://plotdigitizer.com/app NOTE: THE X SCALE ON THIS GRAPH IS NOT
// CONSTANT, WHY????
const struct {
  uint16_t power_mw;     // The actual power in mW, for reference
  int8_t radiolib_value; // The value to pass to RadioLib's setOutputPower()
                         // function
...
void lora_setup(FrequencyBands band, LoRaTxMode mode, uint8_t id,
                bool respect_power_limit) {
  ...
  if (mode == TX_DUTY && EU_868_BANDS[band].polite_access) {
    ...
    tx_mode = TX_DUTY;
    log(S_LORA, T_SYSLOG,
        "polite access not supported in this band, using TX_DUTY instead");
  } else {
    tx_mode = mode;
  }
  ...
  radio.setFrequency(EU_868_BANDS[band].freq_start_mhz +
                     (float)EU_868_BANDS[band].ch_bw_khz /
                         2000.0); // TODO: switch channel
  radio.setBandwidth(EU_868_BANDS[band].ch_bw_khz);

  if (respect_power_limit) {
    // find the closest power value in the table that is less than or equal to
    // the max power
    int8_t radiolib_power = LORA_OUTPUT_POWER_TABLE[0].radiolib_value;
    for (size_t i = 0; i < sizeof(LORA_OUTPUT_POWER_TABLE) /
                               sizeof(LORA_OUTPUT_POWER_TABLE[0]);
         i++) {
      if (LORA_OUTPUT_POWER_TABLE[i].power_mw <=
          EU_868_BANDS[band].max_power_mw) {
...
uint64_t u48le_to_u64(uint8_t u48[6]) {
  return ((uint64_t)u48[0] << 0) | ((uint64_t)u48[1] << 8) |
         ((uint64_t)u48[2] << 16) | ((uint64_t)u48[3] << 24) |
         ((uint64_t)u48[4] << 32) | ((uint64_t)u48[5] << 40);
}
...
static void adjust_time(void *p) {
  ...
  case PKT_DATA: {
    ...
    d->gps.dt = d->gps.dt + base -
                now; // FIXME: GPS dt should be in decisenconds or smt
    break;
  }
  default:
...
void lora_release_tx_packet(void) { xSemaphoreGive(next_packet_mutex); }
...
// Wait for a packet to be received or timeout, return true if packet received,
// false otherwise
...
bool lora_transmit_timeout(void *buffer, uint32_t len, int64_t timeout_ms,
                           LoRaTxMode tx_mode_override) {
  int64_t start_time = millis();
  ...
  case TX_POLITE: {
    ...
    // if the channel is still busy after max_cca attempts, we will transmit
    // anyway
    break;
  }
  ...
  // Wait for the transmission to complete, max timeout is 2*expected time on
  // air
  start_time = millis();
...
void lora_set_rx_cmd_queue(QueueHandle_t handle) { rx_cmd_queue = handle; }
...
LoRaFCState lora_fc_state_machine() {
  ...
  static uint64_t sync_rx_time =
      0; // absolute time of the last sync packet received in ms
  static uint64_t sync_tx_time =
      0; // absolute time of the last sync packet transmitted in ms
  static uint64_t sync_time =
      0; // absolute time of the last sync packet received in ms
  static uint64_t sync_trip_time =
      0; // time it takes for a single sync packet takes to arrive in ms
  ...
  case STATE_DISCONNECTED: {
    ...
    if (lora_receive_timeout(2000) == false)
      break;
  ...
  case STATE_CONNECTING: {
    ...
    if (header.type == PKT_ACCEPT && header.id == LORA_GS_ID) {
      ...
      clock_delta = -((sync_rx_time - sync_tx_time) -
                      (connect_tx_time - connect_rx_time)) /
                    2;
      // FIXME: the single trip time a sync packet takes could be transmitted by
      // the master and the error would be less (just the propagation delay),
      // this instead takes in account the time it takes to transmit the sync
      // packet and the time it takes to receive it back plus the two
      // propagation delays
      sync_trip_time = ((connect_rx_time - sync_tx_time) -
                        (connect_tx_time - sync_rx_time)) /
                       2;
  ...
  case STATE_TRANSMIT: {
    ...
    // If the packet would arrive after the tx window of the fc switch to
    // receive mode
    ...
    lora_transmit_timeout(&tx_packet, sizeof(tx_packet), remaining_time - toa,
                          TX_NONE);
  ...
  case STATE_RECEIVE: {
    // Receive for the ground station window, including the security window to
    // avoid switching too early or loosing packets
    int64_t remaining_time =
        gs_window - slot_relative_time(sync_time, clock_delta);
    ...
    case PKT_SYNC: {
      ...
      int64_t drift =
          sync_rx_time - clock_delta - sync_tx_time - sync_trip_time;
...
LoRaFCState lora_gs_state_machine() {
  static LoRaFCState state = STATE_DISCONNECTED;
  static int64_t sync_sent_time =
      0; // absolute time of the last sync packet sent in ms
  static int64_t connect_rx_time =
      0; // absolute time of the last connect packet received in ms
  static int packets_received = 0; // number of packets received from FC
  static int silent_frames = 0;

  static const int32_t sync_window =
      1000; // time window between sync packets in ms
  ...
  case STATE_DISCONNECTED: {
    ...
    if ((now_ms() - sync_sent_time) >= sync_window) {
      ...
      if (lora_receive_timeout(sync_window -
                               (slot_relative_time(sync_sent_time))) == false) {
        // No connect received within the sync window, return to disconnected
        // state
        break;
      }
  ...
  case STATE_CONNECTING: {
    ...
    if (lora_transmit_timeout(&a, sizeof(a), sync_window / 2, TX_FORCE) ==
        false) {
      state = STATE_DISCONNECTED;
      break;
    }
  ...
  case STATE_TRANSMIT: {
    ...
    // once when entering STATE_TRANSMIT after the first window after the
    // handshake
    ...
    int64_t remaining_time =
        gs_window - security_window - slot_relative_time(sync_sent_time);
  ...
  case STATE_RECEIVE: {
    int64_t remaining_time =
        sync_window - security_window - slot_relative_time(sync_sent_time);
