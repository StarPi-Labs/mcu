...
#define MCU_LOG_BUFFER_SIZE 1024
#endif
 ...
 */
void logf(const std::string_view &prefix, const char *in_format, ...);
} // namespace mcu
...
#define mcu_log_warning(format, ...)                                        \
  mcu::logf(                                                               \
      std::string_view(MCU_LOG_PREFIX_WARNING, sizeof(MCU_LOG_PREFIX_WARNING)),\
      format __VA_OPT__(, ) __VA_ARGS__)
#else
#define mcu_log_warning(format, ...)
#endif
...
#define mcu_log_critical(format, ...)                                       \
  mcu::logf(                                                               \
      std::string_view(MCU_LOG_PREFIX_CRITICAL, sizeof(MCU_LOG_PREFIX_CRITICAL)),\
      format __VA_OPT__(, ) __VA_ARGS__)
#else
#define mcu_log_critical(format, ...)
#endif
