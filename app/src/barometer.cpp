...
// impostato con OSR di 4096, se si vuole + veloce ma meno preciso si abbassa da
// 2^12 a 2^11 o 2^10

float ground_pressure_mbar_1 =
    0.0; // memorizzo pressione misurata sulla rampa di lancio
float ground_pressure_mbar_2 =
    0.0; // memorizzo pressione misurata sulla rampa di lancio
...
miglioramenti se funziona e si vuole upgradare:   1.si può far una media di
30-40 letture per percepire la pressione al suolo, perche ora se ne fa solo 1;
                                                  2.si aumentano il numero di
elementi in coda; 3.si può aggiungere un ciclo di retry se restituisce false lo
status.7

parametri di variazione: 1. Frequenza(ora a 20 hz);
                         2. OSR(ora a 12, quindi 4096 campioni e ci mette circa
9 ms);
                         3. alpha del filtro(ora a 0.15. Se si aumenta è più
sensibile ai cambiamenti di quota, ma dato più tremolante);
...
bool barometer_setup(void)
{
  if (baro1.begin() == false) {
    mcu_log_error("baro", "Barometer 1 " STRINGIFY(BARO1_ADDRESS) " not found");
    return false; // se non funziona segnala l'errore e lascia svolgere le altre
                  // tasks
  }
  if (baro2.begin() == false) {
    mcu_log_error("baro", "Barometer 2 " STRINGIFY(BARO2_ADDRESS) " not found");
    return false; // se non funziona segnala l'errore e lascia svolgere le altre
                  // tasks
  }

  baro1.reset();
  mcu_log_info("baro", "Barometer 1 initialized");
  baro2.reset();
  mcu_log_info("baro", "Barometer 2 initialized");
  ...
  if (status1 == 0) {
    ...
    mcu_log_info("baro", "Barometer 1: Setting ground pressure to {}mbar",
                 ground_pressure_mbar_1);
  } else {
    mcu_log_error("baro", "Barometer 1: Calibration failed with error code {}",
                  status1);
  }
  ...
  if (status2 == 0) {
    ...
    mcu_log_info("baro", "Barometer 2: Setting ground pressure to {}mbar",
                 ground_pressure_mbar_2);
  } else {
    mcu_log_error("baro", "Barometer 2: Calibration failed with error code {}",
                  status2);
  }
...
void barometer_read(BaroData* sample1, BaroData* sample2)
{
  ...
  if (sample1->error_status == 0) {
    ...
    sample1->altitude =
        baro1.getAltitude(ground_pressure_mbar_1); // altitudine in metri
  }
  if (sample2->error_status == 0) {
    ...
    sample2->altitude =
        baro2.getAltitude(ground_pressure_mbar_2); // altitudine in metri
  }
}
