...
// Update the GPSData structure, if new data is available then the correct
// members will be updated, otherwise they will be left as-is
void gps_update(GPSData* data)
{
  ...
  while (Serial2.available() > 0 &&
         millis() - start < GPS_SERIAL_READ_TIMEOUT_MS) {
    parser.encode(Serial2.read());
    got_data = true;
    continue;
  }

  if (got_data == false || data == NULL)
    return;
  ...
  if (parser.time.isUpdated() || parser.date.isUpdated()) {
    if (parser.time.isValid() && parser.date.isValid()) {
      ...
      if (std::abs(gps_time - now) > 1000000ULL) {
        ...
        mcu_log_info("gps", "System time updated to {}", new_time);
      }
    }
  }
