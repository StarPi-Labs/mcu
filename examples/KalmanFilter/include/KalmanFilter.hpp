#include <Arduino.h>
#include <ArduinoEigenDense.h>    //Ho provato ad includere la libreria Eigen "originale", ma dà problemi
...
private:
    ...
    const float a_boost = 15;   // Valore di a che separa la fase di boost da quella di coast !!! ancora da capire quanto vale   !!! ancora da settare
...
public:
    ...
    void predict(float a, float alpha, bool airbrake_trigger) {
        ...
        A(0,1) = dt * cos(alpha);                //alpha è l'angolo di tilt rispetto alla verticale 

        if (!airbrake_trigger) {            
            if (a > a_boost) {                   // Boost  !!! Bisogna valutare le condizioni per capire bene la logica del gain scheduling
            ...
            if (x(1)*cos(alpha) > 0) {          // se la velocità verticale v=x(1)*cos(alpha) è positiva (cioè verso l'alto)
