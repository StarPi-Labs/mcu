...
class CharacteristicCallbacks : public NimBLECharacteristicCallbacks {
  void onRead(NimBLECharacteristic *pCharacteristic,
    ...
    if (pCharacteristic->getUUID().equals(uuid)) {
...
void setup(void) {
   ...
   * called with either of those uuid's it will create the associated class with
   * the correct properties and sizes. However we must cast the returned
  ...
  NimBLECharacteristic *pFoodCharacteristic =
      pBaadService->createCharacteristic("F00D", NIMBLE_PROPERTY::READ |
  ...
  /** Custom descriptor: Arguments are UUID, Properties, max length of the value
   * in bytes */
...
void loop() {
  ...
  if (pServer->getConnectedCount()) {
    ...
    if (pSvc) {
      ...
      std::string newValue = std::format(
          "Fries: {:%T}", std::chrono::steady_clock::now() - bootTime);
      pChr->setValue(newValue);
