...
bool barometer_setup(void) {
  ...
  ground_temperature_k =
      (baro1.getTemperature() + baro2.getTemperature()) / 2.0 + 273.15;
...
void barometer_read(BaroData *sample1, BaroData *sample2) {
  ...
  if (sample1->error_status == 0) {
    ...
    sample1->altitude = compute_altitude(
        sample1->pressure, ground_pressure_mbar_1); // altitudine in metri
  }
  if (sample2->error_status == 0) {
    ...
    sample2->altitude = compute_altitude(
        sample2->pressure, ground_pressure_mbar_2); // altitudine in metri
  }
}
