...
public:
  ...
  //   a : accelerazione verticale misurata in G, rispetto l'asse normale alla
  //   scheda alpha : angolo di tilt rispetto alla verticale (in radianti)
  ...
  void predict(float a, float alpha, bool airbrake_trigger)
  {
    ...
    if (!airbrake_trigger) {
      if (a > a_boost) {
        // Boost  !!! Bisogna valutare le condizioni per capire bene la logica
        // del gain scheduling
      ...
      // Airbrakes
      if (x(1) * cos(alpha) > 0) {
        // se la velocità verticale v=x(1)*cos(alpha) è positiva (cioè verso
        // l'alto)
  ...
  void update(float h)
  {
    ...
    // Forma di Joseph
    P = (Matrix2f::Identity() - K * H) * P *
            (Matrix2f::Identity() - K * H).transpose() +
        K * R * K.transpose();
  }
};